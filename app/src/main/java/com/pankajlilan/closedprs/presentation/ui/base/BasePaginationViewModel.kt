 ...
 */
abstract class BasePaginationViewModel<T : DataSource.Factory<Int, K>, K : Any> : ViewModel() {
    ...
    protected fun getListener(): OnDataSourceLoading {
        return object : OnDataSourceLoading {
            override fun onFirstFetch() {
                Log.d("pankajlilandx001", "inside onFirst fetch")
                showRecyclerLoading()
            }

            override fun onFirstFetchEndWithData() {
                Log.d("pankajlilandx001", "inside onFirstFetchEndWithData")
                hideRecyclerLoading()
                hideEmptyVisibility()
            }

            override fun onFirstFetchEndWithoutData() {
                Log.d("pankajlilandx001", "inside onFirstFetchEndWithoutData")
                hideRecyclerLoading()
                showEmptyVisibility()
            }

            override fun onDataLoading() {
                Log.d("pankajlilandx001", "inside onDataLoading")
                showRecyclerLoading()
            }

            override fun onDataLoadingEnd() {
                Log.d("pankajlilandx001", "inside onDataLoadingEnd")
                hideRecyclerLoading()
            }

            override fun onInitialError() {
                Log.d("pankajlilandx001", "inside onInitialError")
                hideRecyclerLoading()
                showErrorToast()
            }

            override fun onError() {
                Log.d("pankajlilandx001", "inside onError")
                hideRecyclerLoading()
                showErrorToast()
            }
        }
